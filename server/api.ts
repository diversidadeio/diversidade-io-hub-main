...
apiRouter.post("/ler-logs-empresa", async (req, res) => {
  try {
    ...
    if (modo === "sobre_empresa") {
      ...
      else if (nomeEmpresa && nomeEmpresa.trim().length > 1) q = q.ilike("nome_empresa", `%${nomeEmpresa.trim()}%`);
    ...
    if (modo === "usuarios_empresa") {
      let idResolvido = empresaId ?? null;
      if (!idResolvido && nomeEmpresa && nomeEmpresa.trim().length > 1) {
        const { data: empresasEncontradas } = await supabaseAdmin.from("empresas").select("id").ilike("razao_social", `%${nomeEmpresa.trim()}%`).limit(5);
        if (!empresasEncontradas || empresasEncontradas.length === 0) {
          const { data: porFantasia } = await supabaseAdmin.from("empresas").select("id").ilike("nome_fantasia", `%${nomeEmpresa.trim()}%`).limit(5);
  ...
  const emailsSemPerfil = emails.filter((e) => {
      ...
      for (const pr of (presencas || [])) {
        ...
        if (u && pr.last_seen) {
          ...
          // Se o ping (presence) for mais recente que o último log oficial, ele atualiza a última atividade!
    ...
    console.error("Erro no endpoint /usuarios-online:", err);
    return res.status(500).json({ erro: err.message });
  }
});
...
apiRouter.get("/ler-logs-metricas", async (req, res) => {
  try {
    ...
    // Filtrar falhas que foram apenas de e-mails não cadastrados
...
apiRouter.post("/busca-ia", async (req, res) => {
  try {
    ...
    // Etapa 1: filtro nos campos principais (mais relevantes para identidade da empresa)
    ...
    const filtrosPrimarios = palavrasChave.length > 0
      ? palavrasChave.flatMap((p) => camposPrimarios.map((c) => `${c}.ilike.%${p}%`)).join(",")
      : "";

    // Etapa 2 (ampliação): inclui sobre_empresa e area_geografica
    const filtrosTodos = palavrasChave.length > 0
      ? palavrasChave.flatMap((p) => camposTextuais.map((c) => `${c}.ilike.%${p}%`)).join(",")
      : "";
    ...
    let { data: empresasFiltradas, error: erroEmpresasData } = palavrasChave.length > 0
      ? await baseQuery.or(filtrosPrimarios)
      : await baseQuery;
    ...
    if (palavrasChave.length > 0 && (empresasFiltradas?.length ?? 0) < 10) {
      ...
      const { data: empresasAmpladas } = await queryAmpliada.or(filtrosTodos);
...
/**
 * POST /api/atualizar-cnaes
 * Busca os CNAEs da BrasilAPI e atualiza no banco.
 * Body: { empresa_id: string, cnpj: string }
 */
apiRouter.post("/atualizar-cnaes", async (req, res) => {
  try {
    const { empresa_id, cnpj } = req.body;
    if (!empresa_id || !cnpj) {
      return res.status(400).json({ erro: "empresa_id e cnpj são obrigatórios." });
